...
MobilePhoneManager::Return<std::vector<std::string>> MobilePhoneManager::get_phone_options(const std::string& sys_user_id) {
    ...
    std::string message;
    const repository::MobilePhoneDAO mobile_phone_dao;
    if (const repository::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_list(sys_user_id, phone_numbers);
        repository::DaoStatus::Success != ret_mobile_phone_dao ) {
        message = "发生错误";
    }else {
        message = "成功";
    }
...
MobilePhoneManager::Return<std::vector<models::MobilePhone>> MobilePhoneManager::get_mobile_phones(const std::string& sys_user_id, const std::string& telecom_operator, const std::string& phone_number) {
    ...
    std::string message;
    const repository::MobilePhoneDAO mobile_phone_dao;
    if (const repository::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_all(sys_user_id, telecom_operator, phone_number, phones);
        repository::DaoStatus::Success != ret_mobile_phone_dao) {
        message = "发生错误";
    }else {
        message = "成功";
...
MobilePhoneManager::Return<models::MobilePhone> MobilePhoneManager::get_phone_by_owner_and_id(const std::string& sys_user_id, const int phone_id) {
    ...
    const repository::MobilePhoneDAO mobile_phone_dao;
    if (const repository::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_by_id(phone_id, temp_phone);
        repository::DaoStatus::Success != ret_mobile_phone_dao) {
        return {phone, "发生错误"};
    }
...
MobilePhoneManager::Return<models::MobilePhone> MobilePhoneManager::get_phone_by_id(const int phone_id) {
    ...
    const repository::MobilePhoneDAO mobile_phone_dao;
    if (const repository::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.find_by_id(phone_id, phone);
        repository::DaoStatus::Success != ret_mobile_phone_dao) {
        return {phone, "发生错误"};
    }
    return {phone, "成功"};
    ...
    const std::string& postscript)
{
    ...
    if (
        const repository::DaoStatus ret_mobile_phone_dao = mobile_phone_dao.add(new_phone);
        repository::DaoStatus::Success != ret_mobile_phone_dao) {
        message = "发生了错误";
        ret = false;
    }else {
    ...
    const std::string& postscript)
{
    ...
    const repository::DaoStatus ret_update = mobile_phone_dao.update(id,owner_id,
        phone_number,telecom_operator,service_pwd,pin,puk,
        join_time,phone_area,postscript,current_date_time);

    if (
        repository::DaoStatus::Success != ret_update
    ) {
        message = "发生错误";
        ret = false;
    }else {
...
MobilePhoneManager::Return<bool> MobilePhoneManager::delete_mobile_phone(const int id, const std::string& user_id) {
    ...
    const std::string current_date_time = utils::DateTime::getCurrentDateTime();
    if (
        const repository::DaoStatus ret_status = mobile_phone_dao.remove(id, user_id);
