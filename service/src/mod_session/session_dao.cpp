...
namespace zinpass::mod_session {
    SessionDAO::SessionDAO() : pool_(repository::ConnectionPool::get_instance()) {
    }

    SessionDAO::~SessionDAO() = default;

    repository::DaoStatus SessionDAO::find_by_id(const std::string& id, mod_session::Session& session) const {
        ...
        sqlite3_finalize(stmt);
        pool_.release_connection(conn);
        return repository::DaoStatus::Success;
    }
    ...
    repository::DaoStatus SessionDAO::add(const mod_session::Session& session) const {
        sqlite3* conn = pool_.get_connection();
        ...
        repository::SQLDebug::log_sql(stmt, true);
        repository::DaoStatus status;
        const int result = sqlite3_step(stmt);
        if (SQLITE_DONE == result) {
            status = repository::DaoStatus::Success;
            ...
            case SQLITE_CONSTRAINT: {
                status = repository::DaoStatus::DuplicateKey; // 约束冲突（唯一键、外键、非空约束等）
            ...
            case SQLITE_BUSY: {
                status = repository::DaoStatus::DatabaseBusy; // 数据库被锁定（如其他连接持有写锁）
                utils::LogManager::AddLog(
            ...
            case SQLITE_READONLY: {
                status = repository::DaoStatus::ReadOnlyError; // 数据库文件只读或权限不足
                utils::LogManager::AddLog(
            ...
            case SQLITE_MISUSE: {
                status = repository::DaoStatus::ConfigurationError; // API使用错误（如未正确关闭stmt）
                utils::LogManager::AddLog(
            ...
            case SQLITE_ERROR:
            default: {
                status = repository::DaoStatus::GenericError; // 通用错误（如数据类型转换失败、SQL语法错误等）
                utils::LogManager::AddLog(
    ...
    repository::DaoStatus SessionDAO::update(const mod_session::Session& session) const {
        sqlite3* conn = pool_.get_connection();
        ...
        if (SQLITE_DONE != sqlite3_step(stmt)) {
            ...
            sqlite3_finalize(stmt);
            pool_.release_connection(conn);
            return repository::DaoStatus::GenericError;
        }
    ...
    repository::DaoStatus SessionDAO::remove(const std::string& id) const {
        sqlite3* conn = pool_.get_connection();
    ...
    repository::DaoStatus SessionDAO::update_expiration(const std::string& id, const std::string& expires_date_time) const{
        sqlite3* conn = pool_.get_connection();
